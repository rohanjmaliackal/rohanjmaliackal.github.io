                                   ...
                                   <!-- <div class="search_div">
                                    ...
                                    <table id="myTable" class=" pagination  searchable  table_style_3 css_class">
                                        ...
                                        <?php
                                            ...
                                            $query_select_leave=mysqli_query($con,"select * from leaves,doctor where leaves.status='0' and doctor.status='0' and leaves.doctor_id=doctor.doctor_id order by leaves.leave_id DESC");
                                            if(mysqli_fetch_row($query_select_leave)>0){
                                                $i=0;
                                                $query_select_leave=mysqli_query($con,"select * from leaves,doctor where leaves.status='0' and doctor.status='0' and leaves.doctor_id=doctor.doctor_id order by leaves.leave_id DESC");	
                                                while($row_leave=mysqli_fetch_array($query_select_leave)){
                                                    $i++;
                                                    ?>
                                                    <tr>
                                                        <td><?php echo $i; ?></td>
                                                        <td><?php echo $row_leave['specialist']; ?></td>
                                                        <td><?php echo $row_leave['name']; ?></td>
                                                        <td><?php echo $row_leave['date']; ?></td>
                                                        <th>
                                                            <a href="delete-leave.php?leave_id=<?php echo $row_leave['leave_id']; ?>" class="btn  btn_create upload-result"   style="background: rgb(118, 60, 60);width:35%;float:left;color: #fff;" onclick="return myDelete();">Delete</a>
